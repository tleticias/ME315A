...
`Resposta`: Os dois primeiros QUERYS tiveram as tabelas com os mesmos resultados, isso porque o DISTINCT analisa a linha inteira que engloba as variáveis DNO e SALARY e, como não haviam linhas identicas na tabela original, não houve mudança ao utilizar o DISTINCT. O terceiro QUERY contém as mesmas informações que das 2 tabelas anteriores, porém com a ordenação comandada pela variável DNO.
...
`Resposta:` Apenas a primeira e a terceira cláusula são logicamente equivalentes, pois a negação de "AND" é "OR", então a segunda cláusula deveria ser `WHERE NOT ACRES < 50 OR NOT STATE = 'MA'`
...
`Resposta:` Apenas a primeira e a terceira cláusula são logicamente equivalentes, pois para a segunda ser equivalente, deveria ser "AND" ao invés de "OR"
